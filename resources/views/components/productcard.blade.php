    ...
    <a href="{{ url('productdetail/'.$product->ProductID) }}" style="text-decoration: none; color: black;">
        <div class="card">
            ...
            <div class="d-flex">
                ...
                <div class="mt-1 text-primary" style="font-size: 18px;margin-left: 5px">
                    @if(count($product->reviews) >= 1)
                    {{ number_format($product->reviews->avg('reviewscore'), 1) }} <span class="text-dark">({{ count($product->reviews) }})</span>
                    @endif
                </div>
            </div>
